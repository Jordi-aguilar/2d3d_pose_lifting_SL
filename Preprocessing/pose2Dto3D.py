...
def initialization(Xx, Xy, Xz, Xw, structure, sigma, randomNubersGenerator, dtype, percentil=0.5):
    T = Xx.shape[0]
    n = Xx.shape[1]
    
    nLines, nPoints = skeletalModel_new.structureStats(structure)
    ...
    Ls = {}
    # calculate length of each Bone. It is done taking the 50 percentile of all the lengths
    for iBone in range(len(structure)):
        ...
        for t in range(T):
            ...
            wb = Xw[t, b]
            w = min([wa, wb]);
            L = norm([ax - bx, ay - by, az - bz])
            Ls[line].append(L)
    # compute logarithm of each length
    for i in range(len(lines)):
        lines[i] = math.log(perc(Ls[i], percentil))
    
    # for each bone: compute "anglex, angley, anglez", which gives us the vector to the next bone
    # NOTE THAT THE LOOP STARTS WITH THE BONES AND THEN WITH THE FRAMES. USING THE BACKPROPAGATION IDEA. (begin with the head)
    for iBone in range(len(structure)):
        a, b, line = structure[iBone]
        ...
        for t in range(T):
            ...
            #anglex, angley, anglez = computeB(ax, ay, az, tx, ty, tz, L)
            anglex, angley, anglez = (tx - ax, ty - ay, tz - az) # new
    ...
    return lines, rootsx, rootsy, rootsz, anglesx, anglesy, anglesz, Yx, Yy, Yz, w  # Added w
