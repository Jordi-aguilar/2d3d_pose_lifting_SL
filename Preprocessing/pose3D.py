...
def backpropagationBasedFiltering(
    ...
    nBones, nPoints = skeletalModel_new.structureStats(structure)
    #nBones, nPoints = (25, 50)
    ...
    tarz = tf.placeholder(dtype=dtype) # new
    ...
    # weighted MSE
    loss = tf.reduce_sum(w * tf.square(x - tarx) + w * tf.square(y - tary) + w * tf.square(z - tarz)) / (T * nPoints) # Added w * tf.square(z - tarz)
    ...
    for iCycle in range(nCycles):
        sess.run(train, {tarx: tarx_values, tary: tary_values, tarz: tarz_values, w: w_values}) # tarz: tarz_values,
        print("iCycle = %3d, loss = %e" % (iCycle, sess.run([loss], {tarx: tarx_values, tary: tary_values, tarz: tarz_values, w: w_values})[0])) # tarz: tarz_values,
